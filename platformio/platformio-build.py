...
sys.path.insert(
    0,
    join(FRAMEWORK_DIR, "platformio", "package_deps", "py%d" % sys.version_info.major),
)
...
def process_path(dirs):
    ...
    for d in dirs:
        ...
        if "windows" in util.get_systype() and "idedata" not in COMMAND_LINE_TARGETS:
...
if MBED_RTOS:
    ...
    env.Append(CPPPATH=[join(FRAMEWORK_DIR, "cmsis", "TARGET_CORTEX_M")])
...
env.BuildSources(
    join("$BUILD_DIR", "FrameworkMbed"), FRAMEWORK_DIR, src_filter=src_filter
)
...
def merge_firmwares(target, source, env):
    ...
    if framework_processor.has_target_hook():
        ...
        framework_processor.apply_hook(
            env.subst(join("$BUILD_DIR/$PROGNAME$PROGSUFFIX")), firmware_file
        )
        ...
        env.Append(
            EXTRA_LIB_BUILDERS=[
                PlatformIOLibBuilder(
                    env, join(FRAMEWORK_DIR, lib), get_dynamic_manifest(lib)
                )
            ]
        )
