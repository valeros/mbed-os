...
def process_paths(paths):
    result = []
    for p in paths:
        path = os.path.join(FRAMEWORK_DIR, p)
...
def _file_long_data(env, data):
    ...
    tmp_file = os.path.join(
        build_dir, "longincs-%s" % hashlib.md5(hashlib_encode_data(data)).hexdigest()
    )
...
# significant amount of time for SCons to scan for changes in that paths.
...
env.Append(
    ...
    LIBPATH=process_paths(configuration.get("lib_paths")),
